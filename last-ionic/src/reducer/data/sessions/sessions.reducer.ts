...
export const sessionsReducer = (state: ConfState, action: SessionsActions): ConfState => {
  switch (action.type) {
    ...
    case 'set-search-string':       return { ...state, searchString:    action.searchString }
    case 'set-menu-enabled':        return { ...state, menuEnabled:     action.menuEnabled }
    case 'set-filter':              return { ...state, filter:          action.filter }
    case 'set-filter-date':         return { ...state, filterDate:      action.filterDate }
    case 'set-order-field':         return { ...state, orderField:      action.orderField }
    case 'set-search-order':        return { ...state, searchOrder:     action.searchOrder }
  }
}
