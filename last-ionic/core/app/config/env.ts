...
export function getYupLocaleConfig(t: TFunction) {
  return {
    mixed: {
      default: t('mixed.validation.invalid', { defaultValue: 'Invalid' }),
      required: t('mixed.required.key', { defaultValue: 'Required' }),
    },
    number: {
      min: ({ min }: { min: number }) => t('number.min.key', { min, defaultValue: `Minimal: ${min}` }),
      max: ({ max }: { max: number }) => t('number.max.key', { max, defaultValue: `Maximal: ${max}` }),
    },
    string: {
      email: t('string.email.key', { defaultValue: 'Must be an email' }),
      min: ({ min }: { min: number }) => t('string.min.key', { min, defaultValue: `Minimum length: ${min}` }),
      max: ({ max }: { max: number }) => t('string.max.key', { max, defaultValue: `Maximum length: ${max}` }),
    },
  };
}
